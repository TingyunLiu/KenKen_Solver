(module a10 scheme
  ...
  ;; You may tweak the next three constants depending on your
  ;; operating system and screen resolution

  ;; the size of the cage constraints (e.g., 5x or 3-)
  (define font-exp (make-object font% 5 'default))
  
  ;; the size of the numbers in the solution
  (define font-val (make-object font% 40 'default))

  ;; the size of each square holding the number and constraint
  ...
  (define (puzzle-setup puzz draw-option)
    (cond 
      [(not (symbol=? draw-option 'off))
       (begin
         (set! blank-board (get-board puzz))
